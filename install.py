...
with Halo(text='\033[1m\033[33mInstalling Required Files!\033[0m', spinner='dots'):
    subprocess.call('pkg install tur-repo -y >/dev/null 2>&1 && pkg install x11-repo -y >/dev/null 2>&1 && pkg install root-repo -y >/dev/null 2>&1 && pkg install python -y >/dev/null 2>&1 && pip install Cython pip >/dev/null 2>&1 && pkg install termux-api -y >/dev/null 2>&1 && pip install tqdm >/dev/null 2>&1 && pip install termux-api -y >/dev/null 2>&1 && pkg install openssh -y >/dev/null 2>&1 && pkg install figlet >/dev/null 2>&1 && pkg install toilet -y >/dev/null 2>&1 && pkg install grep -y >/dev/null 2>&1 && pkg install termux-am -y >/dev/null 2>&1 && pkg install termux-tools -y >/dev/null 2>&1 && pkg install wireless-tools -y >/dev/null 2>&1 && pkg install unzip -y >/dev/null 2>&1', shell=True)
